...
<?php
    ...
    function edit_donor()
    {
        if(!empty($_POST['name']) && !empty($_POST['address']) && !empty($_POST['area']) && !empty($_POST['sub-area']) && !empty($_POST['branch']) && !empty($_POST['bg']) && !empty($_POST['nid']) && !empty($_POST['phone']) && !empty($_POST['email']) && !empty($_POST['schedule'])) 
        {
            ...
            global $e;
            $query = "UPDATE donor SET b_id ='$branch', d_name ='$name', address ='$address', area='$area', subarea ='$sub_area', national_id ='$nid', blood_group ='$bg', phone ='$phone', email ='$email', schedule = TO_DATE('$schedule', 'DD-MM-YYYY hh:mi:ss AM') WHERE d_id =".$e;
